cd ~/work/core/pipelines/alpha/crm/crm_plus && TPL=crm_plus_contact_kdm_audit_bronze/crm_plus_contact_kdm_audit_bronze.yaml; while read -r t f; do mkdir -p "$t"; sed -e "s|crm_plus_contact_kdm_audit_bronze|$t|" -e "s|function: contact_kdm_audit_bronze|function: $f|" "$TPL" > "$t/$t.yaml"; done <<'EOF'
...
cat ~/work/core/pipelines/alpha/crm/crm_plus/crm_plus_emp_info_bronze/crm_plus_emp_info_bronze.yaml
